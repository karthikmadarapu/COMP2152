...
def two_sum_optimized(numbers, target):
    ...
    # Loop through numbers,checking if nedded value exists in seen
    for i in range(len(numbers)):
        needed = target - numbers[i]
        if needed in seen:
            return [seen[needed], i]
        seen[numbers[i]] = i  # Store the current number and its index in seen
    return None
